...
def _styles():
    ...
    return {
        "label": ParagraphStyle("label", fontSize=7, textColor=ACCENT,
                                spaceAfter=2, leading=10),
        "title": ParagraphStyle("title", fontSize=24, textColor=WHITE,
                                fontName="Helvetica-Bold", spaceAfter=4, leading=28),
        "subtitle": ParagraphStyle("subtitle", fontSize=9, textColor=MUTED,
        ...
        "kpi_name": ParagraphStyle("kpi_name", fontSize=7, textColor=MUTED,
                                   leading=10),
        "kpi_value": ParagraphStyle("kpi_value", fontSize=14, textColor=WHITE,
                                    fontName="Helvetica-Bold", leading=18),
        "kpi_delta": ParagraphStyle("kpi_delta", fontSize=8, leading=10),
        "bullet": ParagraphStyle("bullet", fontSize=9, textColor=TEXT,
                                 leading=14, leftIndent=10, spaceAfter=3),
        "tag": ParagraphStyle("tag", fontSize=8, textColor=TEXT,
...
def _tone_color(ton: str) -> colors.Color:
    return {"optimiste": GREEN, "neutre": YELLOW, "pessimiste": RED}.get(ton.lower(), YELLOW)
...
def _header(s, filename: str, ton: str, raison: str) -> list:
    ...
    header_data = [[
        Paragraph("REPORT READER", ParagraphStyle("brand", fontSize=8, textColor=ACCENT,
                                                   fontName="Helvetica-Bold", leading=10)),
        Paragraph(f"Generated {date.today().strftime('%B %d, %Y')}",
                  ParagraphStyle("date", fontSize=8, textColor=MUTED,
    ...
    # Tone badge inline
    tone_data = [[
        Paragraph("OVERALL TONE", ParagraphStyle("tl", fontSize=7, textColor=MUTED, leading=10)),
        Paragraph(f"● {tone_label}",
                  ParagraphStyle("tv", fontSize=9, textColor=tone_color,
                                 fontName="Helvetica-Bold", leading=12)),
        Paragraph(raison, ParagraphStyle("tr", fontSize=8, textColor=MUTED, leading=12)),
    ]]
    tone_table = Table(tone_data, colWidths=[28*mm, 28*mm, W - 2*MARGIN - 56*mm])
    ...
    elements.append(Spacer(1, 6*mm))
    elements.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#2a2f42")))
    elements.append(Spacer(1, 4*mm))
    return elements
...
def _kpis(s, kpis: list) -> list:
    ...
    for i, kpi in enumerate(kpis):
        ...
        cell = [
            ...
            Paragraph(delta, ParagraphStyle("kd", fontSize=8, textColor=delta_color, leading=10)),
        ]
        row.append(cell)
        if len(row) == 3:
            row_data.append(row)
    ...
    for row in row_data:
        ...
        t.setStyle(TableStyle([
            ...
            ("INNERGRID", (0, 0), (-1, -1), 0.5, colors.HexColor("#2a2f42")),
            ("VALIGN", (0, 0), (-1, -1), "TOP"),
        ]))
        elements.append(t)
...
def _themes(s, themes: list) -> list:
    ...
    elements.append(Paragraph(tags, ParagraphStyle("tags", fontSize=8, textColor=TEXT,
...
def _risks_opps(s, risques: list, opportunites: list) -> list:
    ...
    left_items = [Paragraph("⚠  RISKS", ParagraphStyle("rh", fontSize=8, textColor=RED,
                                                         fontName="Helvetica-Bold", leading=12,
                                                         spaceAfter=4))]
    for r in risques:
        left_items.append(Paragraph(f"• {r}", ParagraphStyle("ri", fontSize=8, textColor=TEXT,
                                                               leading=13, leftIndent=6,
                                                               spaceAfter=3)))

    right_items = [Paragraph("✓  OPPORTUNITIES", ParagraphStyle("oh", fontSize=8, textColor=GREEN,
                                                                  fontName="Helvetica-Bold",
                                                                  leading=12, spaceAfter=4))]
    for o in opportunites:
        right_items.append(Paragraph(f"• {o}", ParagraphStyle("oi", fontSize=8, textColor=TEXT,
                                                                leading=13, leftIndent=6,
                                                                spaceAfter=3)))
...
def _footer(s) -> list:
    return [
        ...
        Spacer(1, 3*mm),
        Paragraph("Generated by Report Reader · Powered by Claude AI", s["footer"]),
    ]
...
def generate_pdf(result: dict, filename: str) -> bytes:
    ...
    story += _header(s, filename, result.get("ton", "neutre"), result.get("raison_ton", ""))
    story += _summary(s, result.get("resume", ""))
    story += _kpis(s, result.get("kpis", []))
    story += _themes(s, result.get("themes", []))
    story += _risks_opps(s, result.get("risques", []), result.get("opportunites", []))
    story += _footer(s)
